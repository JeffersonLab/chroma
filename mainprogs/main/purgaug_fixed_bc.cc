...
#include "update/heatbath/fixed_boundy_slabs.h"
...
namespace Chroma 
{
  ...
  struct MCControl 
  {
    ...
    multi1d<int> fixed_boundary ;
    multi1d<int> fixed_boundary_thickness ;
    unsigned int t_dir ;
  };
  ...
  void read(XMLReader& xml, const std::string& path, MCControl& p) 
  {
    try { 
      ...
      read(paramtop, "./FixedBoundary", p.fixed_boundary);
      read(paramtop, "./FixedBoundaryThick", p.fixed_boundary_thickness);
      read(paramtop, "./TimeDirection", p.t_dir);
		...
		const Subset& bc) 
  {
    ...
    for(int i=0; i < to_do; i++) 
    {
      ...
      mciter_fixed_bc(u, S_g, hb_control.hbitr_params.hb_params,bc); //one hb sweep
	    ...
	    const Subset& bc) 
  {
    ...
    try 
    {
      ...
      {
	doWarmUp(xml_out, u, S_g, hb_control, default_measurements, user_measurements,bc);
	hb_control.mc_control.n_warm_up_updates = 0;  // reset
      }

      // Do the production updates
      doProd(xml_out, u, S_g, hb_control, default_measurements, user_measurements,bc);
    }
    catch( const std::string& e) { 
...
int main(int argc, char *argv[]) 
{
  ...
  multi1d<int> t0 = hb_control.mc_control.fixed_boundary ;
  multi1d<int> th(t0.size()); // thickness has the same size as t0
  for(int i(0);i<t0.size();i++)
    if(i<hb_control.mc_control.fixed_boundary.size())
      th[i] = hb_control.mc_control.fixed_boundary_thickness[i];
    else
      th[i] = 0 ;// thickness is set to zero if length of thickness is less that length of t0
	
  int t_dir =  hb_control.mc_control.t_dir ;
  FixedBoundarySlabSet fixed_boundary(t_dir,t0,th,Layout::lattSize()[t_dir]);
  
  // Run
  try 
  {
    doHB(u, *S_g, hb_control, the_measurements,fixed_boundary.getSet()[1]);
  } 
  catch(std::bad_cast) 
  {
    QDPIO::cerr << "PURGAUG: caught cast error" << std::endl;
