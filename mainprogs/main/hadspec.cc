// $Id: hadspec.cc,v 1.1 2005-07-03 16:15:49 edwards Exp $
/*! \file
 * \brief Main code for hadronic spectrum measurements
 */

#error  "DO NOT USE - REFERENCE FOR NEW INLINE CODE"
...
struct HadSpec_input_t
{
  multi1d<HadSpecParam_t>  param;
  Cfg_t                    cfg;
};
...
//! Input propagator files
void read(XMLReader& xml, const string& path, HadSpecParam_t& input)
{
  XMLReader inputtop(xml, path);

  read(inputtop, "HadSpecParam", input.had_param);
...
void read(XMLReader& xml, const string& path, HadSpec_input_t& input)
{
  XMLReader inputtop(xml, path);
  ...
  {
    QDPIO::cerr << "Error reading hadspec data: " << e << endl;
    QDP_abort(1);
  }
}
...
int main(int argc, char **argv)
{
  ...
  HadSpec_input_t  input;

  // Instantiate xml reader for DATA
  XMLReader xml_in(Chroma::getXMLInputFileName());

  if (input.param.size() == 0)
    QDP_error_exit("hadspec input empty");

  // Read data
  read(xml_in, "/hadspec", input);

  // Specify lattice size, shape, etc.
  Layout::setLattSize(input.param.nrow);
  Layout::create();

  QDPIO::cout << " HADSPEC: Spectroscopy for Wilson fermions" << endl;
