...
int main(int argc, char **argv)
{
  ...
  // Read data
  InlineSpectrumParams input(xml_in, "/spectrum_w");
  InlineSpectrum  meas(input);
  ...
  try
  {
    read(xml_in, "/spectrum_w/Cfg", cfg);
  }
  catch(const string& e)
  {
    QDP_error_exit("Error reading in spectrum_w: %s", e.c_str());
  }
