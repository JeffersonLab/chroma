...
<prop_root> <config_number> <qq_template> <qqq_template>" unless $#ARGV eq 10;
...
open(QQ_LIST, " > qq_tmp");	# List of diquarks
...
system("sort qq_tmp | uniq > qq_list");	# Pulling out unique diquarks
...
#
#  Erase all forward unsmeared propagators
#
...
#
#  Next we need to write out the qq files

open(QQ_LIST, "< qq_list");
while(<QQ_LIST>){
    chomp;
...
#
    ...
    $qq_name = "qq_".$prop_root."_SS";

    for($prop_ctr = 0; $prop_ctr < 2; $prop_ctr++){
...
#
#  Append the source and sink information to the qq name
	$qq_name = Parse_groups::make_prop_name($qq_name, $flavour,
						 $snk_out, $snk_len_out,
						 $src_out, $src_len_out);
    }

    $qq_name =	$qq_name . ".lime$cfg";	# Append configuration number


    open(IN,"<$qq_template");	# Open the template file for read
    while (<IN>){		# Scan through template file
	s/_PROPAGATOR1/$propname[0]/;
	s/_PROPAGATOR2/$propname[1]/;
	s/_DIQUARKNAME/$qq_name/;
...
#  Basically, this repeats the start of the code, 
#  but without the uniq construction
#
...
#  Append the source and sink information to the qq name
	if($prop_ctr < 2){
	    $qqtmp_name = Parse_groups::make_prop_name($qqtmp_name, $flavour,
						    $snk_out, $snk_len_out,
						    $src_out, $src_len_out);
	}
...
#  Append the source and sink information to the qqq name
    ...
    $qqtmp_name = $qqtmp_name . ".lime$cfg";	# Append configuration number


    if ($qqtmp_name eq $qq_name){
...
#
{
    ...
    <elem>
      <Name>ERASE_NAMED_OBJECT</Name>
      <NamedObject>
        <object_id>sh_sh_$qq_name</object_id>
      </NamedObject>
    </elem>
