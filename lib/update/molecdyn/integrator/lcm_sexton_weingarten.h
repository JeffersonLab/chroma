...
#ifndef LCM_SEXTON_WEINGARTEN_H
#define LCM_SEXTON_WEINGARTEN_H
...
namespace Chroma 
{
  ...
  public:
    ...
    // Copy constructor
    LatColMatSextonWeingartenIntegrator(const LatColMatSextonWeingartenIntegrator& l) :
      n_steps(l.n_steps), tau0(l.tau0), H_MD(l.H_MD), n_short_steps(l.n_short_steps), S_short_monomials(l.S_short_monomials), S_long_monomials(l.S_long_monomials) {}

    // ! Destruction is automagic
    ~LatColMatSextonWeingartenIntegrator(void) {};
    ...
    void operator()(AbsFieldState<multi1d<LatticeColorMatrix>,
      ...
      //  U_short
      U_short(dtbyN, n_short_steps,  s);
      
      // Now the main loop 
      for(int i=0; i < n_steps-1; i++) { 
	// Finish previous long timescale half-step, Do new long timescale
	// half step
	U_long(dt, s);
  ...
  protected:
    ...
    inline void U_short(const Real& dtbyN, 
      ...
      // first half step of length dt / (2N)
      leapP(S_short_monomials, dtby2N, s);

      // first full step of length dt / N
      leapQ(dtbyN, s); 
      for(int i=0; i < N-1; i++) { 

	// finish first half step of length dt/(2N)
	// start new half step of lenght dt/(2N) 
	// altoghether a step of length dt/N
	leapP(S_short_monomials, dtbyN, s); 
