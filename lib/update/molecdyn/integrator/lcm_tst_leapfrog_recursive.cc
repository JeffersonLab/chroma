...
{ 
  ...
  namespace LatColMatTSTLeapfrogRecursiveIntegratorEnv 
  {
	...
	LatColMatTSTLeapfrogRecursiveIntegratorParams p(xml, path);
    
	return new LatColMatTSTLeapfrogRecursiveIntegrator(p);
      }
    ...
    const std::string name = "LCM_TST_LEAPFROG";
  ...
  LatColMatTSTLeapfrogRecursiveIntegratorParams::LatColMatTSTLeapfrogRecursiveIntegratorParams(XMLReader& xml_in, const std::string& path) 
  {
	...
	QDPIO::cout << subintegrator_xml << std::endl;
      ...
      QDPIO::cout << "Error reading XML in LatColMatTSTLeapfrogRecursiveIntegratorParams " << e << std::endl;
      QDP_abort(1);
    }
  }
  ...
  void read(XMLReader& xml, 
    ...
    LatColMatTSTLeapfrogRecursiveIntegratorParams tmp(xml, path);
    p = tmp;
  }
  ...
  void LatColMatTSTLeapfrogRecursiveIntegrator::operator()( 
					     ...
					     const Real& traj_length) const
  {
    ...
    // Its sts so:
    subIntegrator(s, dtauby2);  // First half step
    expSdt(s, dtau); 
    for(int i=0; i < n_steps-1; i++) {  // N-1 full steps
       subIntegrator(s, dtau);
       expSdt(s, dtau);
    }
    subIntegrator(s, dtauby2);  // Last Half Step
