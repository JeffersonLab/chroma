// -*- C++ -*-
// $Id: lcm_sts_min_norm2_recursive_dtau.h,v 3.1 2008-11-16 17:04:04 bjoo Exp $
...
#ifndef LCM_MIN_NORM2_RECURSIVE_DTAU_H
#define LCM_MIN_NORM2_RECURSIVE_DTAU_H
...
namespace Chroma 
{
  ...
  {
    LatColMatSTSMinNorm2DTauRecursiveIntegratorParams();
    LatColMatSTSMinNorm2DTauRecursiveIntegratorParams(XMLReader& xml, const std::string& path);
    Real  delta_tau_max;
    Real lambda;
  ...
  public:
    ...
    LatColMatSTSMinNorm2DTauRecursiveIntegrator(Real delta_tau_max_, 
    ...
    LatColMatSTSMinNorm2DTauRecursiveIntegrator(
					 const LatColMatSTSMinNorm2DTauRecursiveIntegratorParams& p) : delta_tau_max(p.delta_tau_max), lambda(p.lambda), SubIntegrator(IntegratorShared::createSubIntegrator(p.subintegrator_xml)) {
