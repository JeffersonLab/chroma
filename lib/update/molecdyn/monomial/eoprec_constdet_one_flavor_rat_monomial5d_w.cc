...
{ 
    ...
    const OneFlavorWilsonTypeFermRatMonomialParams& param) 
  {
    ...
    QDPIO::cout << "Number of pseudoferms= " << num_pf << std::endl;
    //*********************************************************************
    // Fermion action
    {
	...
	dynamic_cast<EvenOddPrecConstDetWilsonTypeFermAct5D<T,P,Q>*>(tmp_act);
      ...
      if( downcast == 0x0 ) {
	QDPIO::cerr << "Unable to downcast FermAct to EvenOddPrecConstDetWilsonTypeFermAct5D in EvenOddPrecConstDetOneFlavorWilsonTypeFermRatMonomial5D()" << std::endl;
	QDP_abort(1);
      }
