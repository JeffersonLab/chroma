...
namespace Chroma
{
  ...
  public:
                                       ...
                                       const InternalFieldsRefreshMode mode)
    {
      ...
      const multi1d<Phi> old_phi = getPhi();
      refreshInternalFields(s);
	...
	QDPIO::cerr << "Failed to cast input Monomial to OneFlavorRatioRatRatExactWilsonTypeFermMonomial " << std::endl;
	QDP_abort(1);
      }
    ...
    {
      multi1d<multi1d<Phi> > new_stack(phi_backup_stack_.size() + 1);
    ...
    virtual void popInternalFields(void)
    {
      if (phi_backup_stack_.size() == 0) {
        QDPIO::cerr << "OneFlavorRatioRatRatExactWilsonTypeFermMonomial::popInternalFields on empty stack" << std::endl;
        QDP_abort(1);
      }
      ...
      multi1d<multi1d<Phi> > new_stack(top);
      for (int i = 0; i < top; ++i) {
        new_stack[i] = phi_backup_stack_[i];
      }
      phi_backup_stack_ = new_stack;
  ...
  private:
    multi1d<multi1d<Phi> > phi_backup_stack_;
