...
namespace Chroma
{
  ...
  public:
	...
	QDPIO::cerr << "Failed to cast input Monomial to OneFlavorRatExactWilsonTypeFermMonomial5D" << std::endl;
	QDP_abort(1);
      }
    ...
    virtual void popInternalFields(void)
    {
      if (phi_backup_stack_.size() == 0) {
        QDPIO::cerr << "OneFlavorRatExactWilsonTypeFermMonomial5D::popInternalFields on empty stack" << std::endl;
        QDP_abort(1);
      }
