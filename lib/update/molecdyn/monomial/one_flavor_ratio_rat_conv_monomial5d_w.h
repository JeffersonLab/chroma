...
#ifndef __one_flavor_ratio_rat_conv_monomial5d_w_h__
#define __one_flavor_ratio_rat_conv_monomial5d_w_h__
...
namespace Chroma
{
  ...
  class OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D : public ExactWilsonTypeFermMonomial5D<P,Q,Phi>
  {
  public:
    //! virtual destructor:
	...
	push(xml_out, "OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D");
	...
	// N and D makeup the rat. poly of the M term and P and & makeup the rat.poly of the den term
	//
	// Need
	...
	// SelfIdentification/Encapsultaion Rule
	XMLWriter& xml_out = TheXMLLogWriter::Instance();
	push(xml_out, "OneFlavorRatioRatConvExactWilsonTypeFermMonomial5DRefresh");
                                       ...
                                       const InternalFieldsRefreshMode mode)
    {
      ...
      const multi1d< multi1d<Phi> > old_phi = getPhi();
      refreshInternalFields(s);
      if (old_phi.size() != getPhi().size()) {
        return;
      }
      Real c1, c2;
	...
	try 
	{
	  const OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D<P,Q,Phi>& fm = dynamic_cast< const OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D<P,Q,Phi>& >(m);
    ...
    {
      multi1d< multi1d<Phi> > loaded_phi = getPhi();
    ...
    {
      multi1d< multi1d< multi1d<Phi> > > new_stack(phi_backup_stack_.size() + 1);
    ...
    virtual void popInternalFields(void)
    {
      if (phi_backup_stack_.size() == 0) {
        QDPIO::cerr << "OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D::popInternalFields on empty stack" << std::endl;
        QDP_abort(1);
      }
      ...
      multi1d< multi1d< multi1d<Phi> > > new_stack(top);
      for (int i = 0; i < top; ++i) {
        new_stack[i] = phi_backup_stack_[i];
      }
      phi_backup_stack_ = new_stack;
  ...
  private:
    multi1d< multi1d< multi1d<Phi> > > phi_backup_stack_;
  };
   ...
   */
  template<typename P, typename Q, typename Phi>
  class OneFlavorRatioRatConvExactUnprecWilsonTypeFermMonomial5D : public OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D<P,Q,Phi>
  {
  public:
    //! virtual destructor:
	...
	XMLWriter& xml_out=TheXMLLogWriter::Instance();
	push(xml_out, "OneFlavorRatioRatConvExactUnprecWilsonTypeFermMonomial5D");
   ...
   */
  template<typename P, typename Q, typename Phi>
  class OneFlavorRatioRatConvExactEvenOddPrecWilsonTypeFermMonomial5D : public OneFlavorRatioRatConvExactWilsonTypeFermMonomial5D<P,Q,Phi>
  {
  public:
    //! virtual destructor:
	...
	XMLWriter& xml_out=TheXMLLogWriter::Instance();
	push(xml_out, "OneFlavorRatioRatConvExactEvenOddPrecWilsonTypeFermMonomial5D");
  ...
  class OneFlavorRatioRatConvExactEvenOddPrecConstDetWilsonTypeFermMonomial5D : 
    public OneFlavorRatioRatConvExactEvenOddPrecWilsonTypeFermMonomial5D<P,Q,Phi>
  {
  public:
    //! virtual destructor:
