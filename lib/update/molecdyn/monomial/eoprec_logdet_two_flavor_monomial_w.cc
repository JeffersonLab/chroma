...
{ 
    ...
    const TwoFlavorWilsonTypeFermMonomialParams& param) 
  {
    ...
    QDPIO::cout << "EvenOddPrecLogDetTwoFlavorWilsonTypeFermMonomial: construct " << param.fermact.id << std::endl;
    ...
    if( downcast == 0x0 ) {
      QDPIO::cerr << "Unable to downcast FermAct to EvenOddPrecLogDetWilsonTypeFermAct in EvenOddPrecLogDetTwoFlavorWilsonTypeFermMonomial()" << std::endl;
      QDP_abort(1);
    }
