...
#include "update/molecdyn/monomial/eoprec_exp_clov_logdet_ee_monomial_w.h"
...
{ 
  ...
  namespace EvenOddPrecExpClovLogDetEvenEvenMonomial4DEnv 
  {
    namespace
    {
      ...
      {
	return new EvenOddPrecExpClovLogDetEvenEvenMonomial4D(EvenOddPrecLogDetEvenEvenMonomialParams(xml, path));
      }
    ...
    const std::string name = std::string("N_FLAVOR_EXP_CLOV_LOGDET_EVEN_EVEN_FERM_MONOMIAL");
  ...
  EvenOddPrecExpClovLogDetEvenEvenMonomial4D::EvenOddPrecExpClovLogDetEvenEvenMonomial4D(const EvenOddPrecLogDetEvenEvenMonomialParams& p) : 
