// $Id: inline_ape_smear.cc,v 1.1 2005-04-07 03:23:20 edwards Exp $
/*! \file
...
{ 
  ...
  void read(XMLReader& xml, const string& path, InlineAPESmearParams::Param_t& param)
  {
    ...
    case 2:
      ...
      {
	QDP_error_exit("apesmear: invalid number of ape smearing iterations, link_smear_num = %d", param.link_smear_num);
      }
  ...
  void read(XMLReader& xml, const string& path, InlineAPESmearParams::Ape_t& input)
  {
  ...
  // Reader for out gauge file
  void write(XMLWriter& xml, const string& path, const InlineAPESmearParams::Ape_t& input)
  {
    push(xml, path);

    write(xml, "volfmt", input.volfmt);
    write(xml, "ape_type", input.ape_type);
    write(xml, "ape_file", input.ape_file);
...
//    write(gauge_xml_out, "szin", szin_gauge_header);
    gauge_xml_out << gauge_xml;
    writeGauge(gauge_file_xml_out, gauge_xml_out, u_ape, params.ape.ape_file,
	       params.ape.volfmt, QDPIO_SERIAL);
    // writeSzin(szin_gauge_header, u_ape, params.ape.ape_file);
