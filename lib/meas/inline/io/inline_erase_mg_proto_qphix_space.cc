...
namespace Chroma 
{ 
  namespace InlineEraseMGProtoQPhiXSpaceEnv
  { 
    namespace
    {
	...
	return new InlineMeas(Params(xml_in, path));
      ...
      const std::string name = "ERASE_MG_PROTO_QPHIX_SUBSPACE";
    }
			   ...
			   XMLWriter& xml_out) 
    {
      ...
      if ( TheNamedObjMap::Instance().check(params.named_obj.object_id) ) {
    	  ...
    	  MGProtoHelpersQPhiX::deleteMGPreconditioner(params.named_obj.object_id);
