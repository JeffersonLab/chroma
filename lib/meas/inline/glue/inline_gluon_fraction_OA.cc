/*! \file
 *  \brief Inline gluon momentum fraction operator OA
 *  O_mn = 2 Tr[F_ms F_ns]
 *  O_B = O_44 - 1/3 O_ii
 *
 * Author: Joe Karpie
...
#include "inline_gluon_fraction_OA.h"
...
{ 
  ...
  namespace InlineGluonMomFracOAEnv 
  { 
    namespace
    {
	...
	return new InlineMeas(p);
      ...
      const std::string name = "GLUON_MOM_FRAC_OA";
    }
...
//QDPIO::cout<< "JK NOTES:: CHECK NORMALIZATION OF THE FIELD STRENGTH TENSOR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
	...
	TheNamedObjMap::Instance().getData< multi1d<LatticeColorMatrix> >(params.named_obj.gauge_id);
      ...
      push(xml_out, "GluonMomFracOA");
      write(xml_out, "update_no", update_no);
      int i = params.param.i;
      ...
      if(i==Nd)
        QDP_error_exit("The index i should not be in time direction");
      ...
      {
        if(s != i)
          Oa += 2 * trace(F[3][s] * F[i][s]);
      }

       // Now store the operator to a memory object
      {
        XMLBufferWriter file_xml, record_xml;
        push(file_xml, "Gluon_mom_frac_Oa");
        write(file_xml, "id", int(0));
        write(file_xml, "GluonMomFracOAParams", params.param);
        pop(file_xml);

        // Store the gauge field
        TheNamedObjMap::Instance().create< LatticeComplex >(params.named_obj.obs_id);
        TheNamedObjMap::Instance().getData< LatticeComplex >(params.named_obj.obs_id) = Oa;
      ...
      pop(xml_out); // pop("GluonMomFracOA");
