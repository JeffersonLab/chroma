// $Id: inline_apply_gaugestate.cc,v 3.1 2006-09-21 18:43:27 edwards Exp $
/*! \file
...
{ 
  ...
  { 
    namespace
    {
      ...
      {
	InlineGaugeStateParams p(xml_in, path);
	return new InlineGaugeState(p);
      }
  ...
  //! GaugeState input
  void read(XMLReader& xml, const string& path, InlineGaugeStateParams::Param_t& param)
  {
    ...
    default:
      QDPIO::cerr << "InlineGaugeStateParams::Param_t: " << version 
		  << " unsupported." << endl;
      QDP_abort(1);
    }
  }
  ...
  //! GaugeState output
  void write(XMLWriter& xml, const string& path, const InlineGaugeStateParams::Param_t& param)
  {
    push(xml, path);
  ...
  //! GaugeState input
  void read(XMLReader& xml, const string& path, InlineGaugeStateParams::NamedObject_t& input)
  {
  ...
  //! GaugeState output
  void write(XMLWriter& xml, const string& path, const InlineGaugeStateParams::NamedObject_t& input)
  {
    push(xml, path);
  ...
  // Params
  InlineGaugeStateParams::InlineGaugeStateParams() 
  { 
    frequency = 0; 
  }

  InlineGaugeStateParams::InlineGaugeStateParams(XMLReader& xml_in, const std::string& path) 
  {
    try 
    {
  ...
  void
  InlineGaugeStateParams::writeXML(XMLWriter& xml_out, const std::string& path) 
  {
    push(xml_out, path);
  ...
  void 
  InlineGaugeState::operator()(unsigned long update_no,
			       XMLWriter& xml_out) 
  {
    START_CODE();
