// $Id: inline_apply_fermstate_w.cc,v 3.1 2006-09-21 18:43:27 edwards Exp $
/*! \file
...
{ 
  ...
  { 
    namespace
    {
      ...
      {
	InlineFermStateParams p(xml_in, path);
	return new InlineFermState(p);
      }
  ...
  //! FermState input
  void read(XMLReader& xml, const string& path, InlineFermStateParams::Param_t& param)
  {
    ...
    default:
      QDPIO::cerr << "InlineFermStateParams::Param_t: " << version 
		  << " unsupported." << endl;
      QDP_abort(1);
    }
  }
  ...
  //! FermState output
  void write(XMLWriter& xml, const string& path, const InlineFermStateParams::Param_t& param)
  {
    push(xml, path);
  ...
  //! FermState input
  void read(XMLReader& xml, const string& path, InlineFermStateParams::NamedObject_t& input)
  {
  ...
  //! FermState output
  void write(XMLWriter& xml, const string& path, const InlineFermStateParams::NamedObject_t& input)
  {
    push(xml, path);
  ...
  // Params
  InlineFermStateParams::InlineFermStateParams() 
  { 
    frequency = 0; 
  }

  InlineFermStateParams::InlineFermStateParams(XMLReader& xml_in, const std::string& path) 
  {
    try 
    {
  ...
  void
  InlineFermStateParams::writeXML(XMLWriter& xml_out, const std::string& path) 
  {
    push(xml_out, path);
  ...
  void 
  InlineFermState::operator()(unsigned long update_no,
			      XMLWriter& xml_out) 
  {
    START_CODE();
