...
{ 
  namespace InlineCreateGridVecsEnv 
  {
    ...
    void read(XMLReader& xml, const std::string& path, Params::Param_t& input)
    {
      ...
      read(inputtop, "points", input.points);
  ...
  namespace InlineCreateGridVecsEnv 
  {
    ...
    const std::string name = "CREATE_GRID_VECS";
	...
	*(TheNamedObjMap::Instance().getData< Handle< MapObject<int,EVPair<LatticeColorVector> > > >(params.named_obj.colorvec_id));
      ...
      const int num_points = params.param.points.size();
      const int num_vecs = Nc*params.param.points.size();
	...
	evals[i].weights.resize(phases.numSubsets());
      ...
      //
      Real One = 1;
      Complex cOne = cmplx(One,0);
      ColorVector zeroCV = zero;
      for(int hit(0); hit < params.param.num_orthog; hit++)
	{
	  for(int p(0); p < num_points; p++)
		...
		QDPIO::cout << name << ": Doing colorvec point : "<<p
			    << " with color: "<< c 
			    << " (vector index: "<<indx << " )" 
			    <<" hit no: "<<hit<<std::endl;
		if (hit == 0) {
		  multi1d<int> coord(Nd) ;
		  {
		    int dd=0 ;
		    for (int d(0);d<Nd; d++){
		      if(d!=params.param.decay_dir){
			coord[d]=params.param.points[p][dd];
			dd+=1 ;
		      }
		    }
		  }
		  //put 1 to a given color for the current point 
		  // on every timeslice
		  for(int t=0; t < phases.numSubsets(); ++t){
		    coord[params.param.decay_dir]=t ;
		    pokeSite(evecs[indx], 
			     pokeColor(zeroCV,cOne,c),
			     coord);
		  }
		  // and now do gaussian smearing
		  gausSmear(u_smr, 
			    evecs[indx],
			    params.param.width, 
			    params.param.num_iter, 
			    params.param.decay_dir);
		}
		else {
		  gausSmear(u_smr, 
			    evecs[indx],
		...
		for(int k=0; k < indx; ++k) {
		  ...
		  for(int t(0);t<phases.numSubsets();t++){
		    evecs[indx][phases.getSet()[t]] -= cc[t]*evecs[k];
		  }
		}
		
		multi1d<Double> norm2 = sumMulti(localNorm2(evecs[indx]),phases.getSet());
		
		for(int t=0; t < phases.numSubsets(); ++t) {
		  evecs[indx][phases.getSet()[t]] /= sqrt(norm2[t]);
		}
		  
	      }//color loop and point loop
	}// hit loop
