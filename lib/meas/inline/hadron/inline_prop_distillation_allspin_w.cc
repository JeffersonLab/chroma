...
#include "qdp.h"
#include "fermact.h"
#include "meas/inline/hadron/inline_prop_distillation_allspin_w.h"
...
{ 
  ...
  namespace InlinePropDistillationAllSpinEnv 
  {
    ...
    const std::string name = "PROP_DISTILLATION_ALL_SPIN";
	...
	QDP_abort(1);
      ...
      //
      QDP::MapObjectDisk<KeyPropDistillationAllSpin_t, TimeSliceIO<LatticeColorVectorF> > prop_obj;
      prop_obj.setDebug(0);
	...
	write(file_xml, "id", std::string("propDistillationAllSpin"));
	...
	for(int tt=0; tt < t_sources.size(); ++tt) {
	  ...
	  for(int spin_source=0; spin_source < Ns; ++spin_source) {
	    ...
	    if (1) {
	      ...
	      // Initialize
	      for(auto key = snk_keys.begin(); key != snk_keys.end(); ++key) {
