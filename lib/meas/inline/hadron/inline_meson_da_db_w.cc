...
#include "inline_meson_da_db_w.h"
...
{ 
  ...
  { 
    namespace
    {
      ...
      {
	return new InlineMesDA(InlineMesDAParams(xml_in, path));
      }
    ...
    const std::string name = "MESON_DB_DA";
  ...
  void read(XMLReader& xml, const std::string& path, InlineMesDAParams::Param_t& param)
  {
    ...
    read(paramtop, "z_max",param.z_max);
    read(paramtop, "z_dir",param.z_dir);
  ...
  void write(XMLWriter& xml, const std::string& path, const InlineMesDAParams::Param_t& param)
  {
    push(xml, path);
    ...
    write(xml, "z_max",param.z_max);
    write(xml, "z_dir",param.z_dir);
  ...
  //! Propagator input
  void read(XMLReader& xml, const std::string& path, InlineMesDAParams::NamedObject_t::Props_t& input)
  {
  ...
  //! Propagator output
  void write(XMLWriter& xml, const std::string& path, const InlineMesDAParams::NamedObject_t::Props_t& input)
  {
    push(xml, path);
  ...
  //! Propagator input
  void read(XMLReader& xml, const std::string& path, InlineMesDAParams::NamedObject_t& input)
  {
    XMLReader inputtop(xml, path);
  ...
  //! Propagator output
  void write(XMLWriter& xml, const std::string& path, const InlineMesDAParams::NamedObject_t& input)
  {
    push(xml, path);
  ...
  // Param stuff
  InlineMesDAParams::InlineMesDAParams()
  { 
    frequency = 0; 
  }

  InlineMesDAParams::InlineMesDAParams(XMLReader& xml_in, const std::string& path) 
  {
    try 
    {
  ...
  void
  InlineMesDAParams::write(XMLWriter& xml_out, const std::string& path) 
  {
    push(xml_out, path);
  ...
  namespace 
  {
	...
	try
	  ...
	  {
	    QDPIO::cerr << InlineMesDAEnv::name << ": caught dynamic cast error" 
			<< std::endl;
	    QDP_abort(1);
	  }
	catch (const std::string& e) 
	  {
	    QDPIO::cerr << InlineMesDAEnv::name << ": error message: " << e 
			<< std::endl;
	    QDP_abort(1);
	  }
	...
	try
	  ...
	  {
	    QDPIO::cerr << InlineMesDAEnv::name 
    ...
    struct AllSinkProps_t
    {
      ...
      //! Read all sinks
      AllSinkProps_t(const InlineMesDAParams::NamedObject_t::Props_t& p){
  ...
  // Function call
  void 
  InlineMesDA::operator()(unsigned long update_no,
			    XMLWriter& xml_out) 
  {
  ...
  // Real work done here
  void 
  InlineMesDA::func(unsigned long update_no,
		      XMLWriter& xml_out) 
  {
    START_CODE();
    ...
    {
      QDPIO::cerr << InlineMesDAEnv::name << ": std::map call failed: " << e 
		  << std::endl;
      QDP_abort(1);
    }
    ...
    {
      const InlineMesDAParams::NamedObject_t::Props_t named_obj = params.named_obj.props[p];
	...
	key.snk_lorentz.resize(3); // this holds the gamma z_dir and z displacement
	
	key.snk_lorentz[0]=params.param.gamma ;
 	key.snk_lorentz[1]=params.param.z_dir ;
	

	LatticePropagator q ;  // The quark is displaced
	LatticePropagator qtmp ;
	// loop over forward backward
	for(int fb=1;fb>-2;fb-=2){
	  q=q2 ;
	  QDPIO::cout<<" Forward/Backward: "<<fb<<std::endl;
	  //loop over z-displacements
	  for(int z(0);z<params.param.z_max;z++){//sink
	    key.snk_lorentz[2]=fb*z;
	    if(!((z==0)&&fb==-1)){// do the z=0 shift only for fb=1
	      QDPIO::cout<<"          Doing shift:"<<fb*z<<std::endl ;
		...
		key.src_name    =  params.param.states[s].name + "-" + 
		  ops[oj].name;
		key.snk_name    =  params.param.states[s].name + "-da" ;
		
		latC=trace(Gamma(params.param.gamma)*q*Gamma(src)*aq);
	    ...
	    }// check for skipping z=0&&fb==-1
	    //now shift the quark propagator
	    if(fb>0){//forward shift
	      QDPIO::cout<<"Shifting Forward: "<<fb<<std::endl;
	      qtmp=shift(q,FORWARD,params.param.z_dir);
	      q=u[params.param.z_dir]*qtmp ;
	    }
	    else{//backward shift
	      QDPIO::cout<<"Shifting Backward: "<<fb<<std::endl;
	      qtmp=adj(u[params.param.z_dir])*q;
	      q=shift(qtmp,BACKWARD,params.param.z_dir);
	    }
	...
	qdp_db.close() ;
      ...
      tictoc.stop();
      QDPIO::cout << InlineMesDAEnv::name << ": contraction time = "
    ...
    snoop.stop();
    QDPIO::cout << InlineMesDAEnv::name << ": total time = "
		<< snoop.getTimeInSeconds() 
		<< " secs" << std::endl;

    QDPIO::cout << InlineMesDAEnv::name << ": ran successfully" << std::endl;
