...
#include "meas/inline/hadron/inline_hadron_2pt_w.h"
...
{ 
  ...
  void read(XMLReader& xml, const string& path, InlineHadron2PtEnv::Params::Param_t& param)
  {
  ...
  void write(XMLWriter& xml, const string& path, const InlineHadron2PtEnv::Params::Param_t& param)
  {
    push(xml, path);
  ...
  //! Propagator input
  void read(XMLReader& xml, const string& path, InlineHadron2PtEnv::Params::NamedObject_t& input)
  {
    XMLReader inputtop(xml, path);
  ...
  //! Propagator output
  void write(XMLWriter& xml, const string& path, const InlineHadron2PtEnv::Params::NamedObject_t& input)
  {
    push(xml, path);
  ...
  { 
    ...
    const std::string name = "HADRON_2PT";
	...
	{
	  QDPIO::cerr << InlineHadron2PtEnv::name << ": caught dynamic cast error" 
		      << endl;
	  QDP_abort(1);
	}
	catch (const string& e) 
	{
	  QDPIO::cerr << InlineHadron2PtEnv::name << ": error message: " << e 
		      << endl;
	  QDP_abort(1);
	}
	...
	QDPIO::cout << "Mass = " << s.Mass << endl;
      ...
      void readAllSinks(AllSinkProps_t& s, 
			InlineHadron2PtEnv::Params::NamedObject_t::Props_t sink_pair)
      {
	...
	QDP_abort(1);
      ...
      {
	QDPIO::cerr << InlineHadron2PtEnv::name << ": map call failed: " << e 
		    << endl;
	QDP_abort(1);
      }
	...
	pop(xml_out);  // array element
      ...
      QDPIO::cout << InlineHadron2PtEnv::name << ": ran successfully" << endl;
