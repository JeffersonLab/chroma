// $Id: inline_block_orthog_colorvec.cc,v 3.1 2009-04-11 03:32:46 edwards Exp $
/*! \file
...
{ 
  namespace InlineBlockOrthogColorVecsEnv 
  {
    ...
    //! Propagator input
    void read(XMLReader& xml, const string& path, Params::Param_t::Sources_t& input)
    {
      XMLReader inputtop(xml, path);
    ...
    //! Propagator output
    void write(XMLWriter& xml, const string& path, const Params::Param_t::Sources_t& out)
    {
      push(xml, path);
  ...
  namespace InlineBlockOrthogColorVecsEnv 
  {
    ...
    const std::string name = "BLOCK_ORTHOG_COLORVECS";
	...
	push(xml_out, "BlockOrthogColorVecs");
	...
	Handle< LinkSmearing >
	  linkSmearing(TheLinkSmearingFactory::Instance().createObject(params.param.src.link_smear.id, 
								       linktop,params.param.src.link_smear.path));
	(*linkSmearing)(u_smr);
      }
      catch(const std::string& e){
	...
	for(int g(0);g<params.param.src.spatial_masks.size();g++){
	  ...
	  evecs[count] = peekSpin(chi,0);
	  
	  if(params.param.OrthoNormal){
	    for(int k(0);k<count;k++){
	      multi1d<DComplex> cc = sumMulti(localInnerProduct(evecs[k],evecs[count]),phases.getSet());
	      for(int t(0);t<phases.numSubsets();t++)
		evecs[count][phases.getSet()[t]] -= 
		  cc[t]*evecs[k] ;
	    }
	    multi1d<Double> norm2 = 
	      sumMulti(localNorm2(evecs[count]),phases.getSet());
	    for(int t(0);t<phases.numSubsets();t++)
	      evecs[count][phases.getSet()[t]] /= sqrt(norm2[t]) ;
	  }
	...
	for(int i(0);i<Nvecs;i++){
	  for(int k(0);k<i;k++){
	    ...
	    for(int b(0);b<blocks.numSubsets();b++) {
	      evecs[i][blocks[b]] -= cc[b]*evecs[k] ;
	    }
	  }
	  multi1d<Double> norm2 =
	    sumMulti(localNorm2(evecs[i]),blocks);
	  for(int b(0);b<blocks.numSubsets();b++) {
	    evecs[i][blocks[b]] /= sqrt(norm2[b]) ;
	  }
	}
	...
	for(int m=0; m < source_corrs.size(); ++m)
	  source_corrs[m] = sumMulti(localNorm2(evecs[m]), 
				     phases.getSet());
	...
	for(int i(0);i<Nvecs;i++){
	  LatticeColorVector Svec = evecs[i] ;
	  (*Smearing)(Svec, u_smr);
	  multi1d<DComplex> cc = 
	...
	color_vecs.insert(i,evecs[i]);
      ...
      pop(xml_out);  // BlockOrthogColorVecs
