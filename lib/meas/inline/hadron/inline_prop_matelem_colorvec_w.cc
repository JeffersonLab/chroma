...
{ 
  namespace InlinePropMatElemColorVecEnv 
  {
	...
	*(TheNamedObjMap::Instance().getData< Handle< QDP::MapObject<KeyPropColorVec_t,LatticeFermion> > >(params.named_obj.prop_id));
      ...
      QDPIO::cout << "Source and prop std::map successfully found and parsed" << std::endl;
	...
	for(int tt=0; tt < t_sources.size(); ++tt)
	{
	  ...
	  // Another NOTE: the colorvec_source is pulled all the way out as far as possible.
          // This means there is the minimal number of lookups of the fermion solution std::vector.
	  ...
	  for(int spin_source=0; spin_source < Ns; ++spin_source)
	  {
		...
		for(int spin_sink=0; spin_sink < Ns; ++spin_sink)
		{
	      ...
	      {
		EVPair<LatticeColorVector> vec_sink; 
		if (eigen_source.get(colorvec_sink,vec_sink) != 0)
		{
		  QDPIO::cerr << __func__ << ": error retrieving eigenvec" << std::endl;
		  QDP_abort(1);
		}
