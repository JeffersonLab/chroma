...
#include "inline_disco_prob_3d_defl_superb_w.h"
...
namespace Chroma
{
  ...
  namespace InlineDiscoProb3dDeflSuperb
  {
    ...
    //! Propagator output
    void write(XMLWriter& xml, const std::string& path,
	       const InlineDiscoProb3dDeflSuperb::Params::NamedObject_t& input)
    {
      push(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path,
	      InlineDiscoProb3dDeflSuperb::Params::Param_t& param)
    {
      XMLReader inputtop(xml, path);
	...
	read(inputtop, "mom_list", param.mom_list);
      ...
      read(inputtop, "t_sources", param.t_sources);
      if (param.t_sources.size() != 1)
	throw std::runtime_error("the tag `t_sources` should have one element");
    ...
    //! Propagator output
    void write(XMLWriter& xml, const std::string& path,
	       const InlineDiscoProb3dDeflSuperb::Params::Param_t& param)
    {
      push(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoProb3dDeflSuperb::Params& input)
    {
      InlineDiscoProb3dDeflSuperb::Params tmp(xml, path);
      input = tmp;
    }

    //! Propagator output
    void write(XMLWriter& xml, const std::string& path,
	       const InlineDiscoProb3dDeflSuperb::Params& input)
    {
      push(xml, path);
    ...
    const std::string name = "DISCO_PROBING_3D_DEFLATION_SUPERB";
	...
	QDP_abort(1);
      ...
      QDPIO::cout << "Generating a " << params.param.probing_distance
		  << "-distance coloring with a power " << params.param.probing_power << std::endl;
      ...
      {
	// Get spatial lattice dimensions
	std::array<int, 4> latt_size{Layout::lattSize()[0], Layout::lattSize()[1],
				     Layout::lattSize()[2], 1};
	coloring.reset(new Coloring(
	  std::vector<std::array<int, 4>>{{{}}, {0, 0, params.param.probing_distance, 0}},
	  params.param.probing_power, latt_size));
      }
	...
	auto urnd =
	  SB::Tensor<4, typename SB::Complex::value_type>("xyzX", SB::latticeSize<4>("xyzX"));
	urand(urnd);
	auto noise_vec =
	  urnd.transformWithCPUFun<SB::Complex>([=](const typename SB::Complex::value_type& x) {
	    return SB::Complex{
	      static_cast<typename SB::Complex::value_type>(std::cos(M_PI / 2 * x)),
	      static_cast<typename SB::Complex::value_type>(std::sin(M_PI / 2 * x))};
	  });
	     ...
	     k1 < max_color; k1 += dk, dk = std::min(max_color - k1, N_rhs))
	{
	  ...
	  {
	    auto hada_mask = SB::fillLatticeField<5, SB::Complex>(
	      "xyztX", {{'t', params.param.t_sources.at(0)}}, {{'t', 1}}, {}, SB::OnDefaultDevice,
	      [&](SB::Coor<4> c) {
		return coloring->getColor(c) == k1 + i_v ? SB::Complex{1} : SB::Complex{0};
	      });
	    LatticeComplex rv = zero;
	    SB::contract<5>(noise_vec, hada_mask, "")
	      .copyTo(SB::asTensorView(rv).kvslice_from_size({{'t', params.param.t_sources.at(0)}},
							     {{'t', 1}}));
