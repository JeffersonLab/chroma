...
namespace Chroma
{
  ...
  namespace SFWaveQuarkSourceConstEnv
  {
    namespace
    {
      ...
      //! Name to be used
      const std::string name("SF_WAVE_SOURCE");
    ...
    void Params::writeXML(XMLWriter& xml, const std::string& path) const
    {
      ...
      write(xml, "SourceType", SFWaveQuarkSourceConstEnv::name);
    ...
    {
      QDPIO::cout << "SF Wave source" << std::endl;
	...
	{
	  multi1d<int> tt(Nd); tt=0 ;
	  phase =  singlePhase(tt,params.mom);
	}
	// Create the source
	for(int color_source = 0; color_source < Nc; ++color_source)
	{
	  ...
	  {
	    // Wave fill a fermion source. Insert it into the propagator source
	    LatticeFermion tmp1;
	    walfil(tmp1, 
		   params.t_source,
		   params.j_decay, 
		   color_source, spin_source);
	    tmp1=phase*tmp1 ;// tmp1 is 1 on the source zero everywhere else
	    //now tmp1 contains the phase
	    LatticeFermion chi;
