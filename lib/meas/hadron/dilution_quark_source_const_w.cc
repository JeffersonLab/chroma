// $Id: dilution_quark_source_const_w.cc,v 1.1 2007-12-14 06:53:42 edwards Exp $
/*! \file
...
{ 
  ...
  { 
    ...
    void read(XMLReader& xml, const string& path, DilutionQuarkSourceConstEnv::Params::QuarkFiles_t::TimeDilutions_t::SpinDilutions_t& input)
    {
    ...
    void write(XMLWriter& xml, const string& path, const DilutionQuarkSourceConstEnv::Params::QuarkFiles_t::TimeDilutions_t::SpinDilutions_t& input)
    {
      push(xml, path);
  ...
  { 
    ...
    bool registerAll() 
    {
      ...
      if (! registered)
      {
	success &= TheFermDilutionOperatorFactory::Instance().registerObject(name, createMeasurement);
	registered = true;
      }
      return success;
    }
...
//------------------------------------------------------      
	...
	for(int ord = 0; ord < creat_oper.orderings.size(); ++ord)
	{
	  ...
	  {
	    QDPIO::cout<<" evalution time dilution element : "<< t0 <<endl; 
