...
{ 
					...
					const CloverFermActParams& param_)
  {
    ...
    // Check we are in 4D
    if ( Nd != 4 ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) only works in 4D" << std::endl;
      QDP_abort(1);
    }
    
    // Check Aniso Direction -- has to be 3.
    if ( param.anisoParam.anisoP== true && param.anisoParam.t_dir != 3 ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) is hardwired for t_dir=3"<< std::endl;
      QDP_abort(1);
    }
    ...
    if( t_size[3] != s_size[3] ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) needs time to be local" << std::endl;
      QDP_abort(1);
    }
