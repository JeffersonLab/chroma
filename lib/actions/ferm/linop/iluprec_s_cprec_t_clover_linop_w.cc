// $Id: iluprec_s_cprec_t_clover_linop_w.cc,v 3.4 2008-01-02 17:05:54 bjoo Exp $
/*! \file
...
{ 
					...
					const CloverFermActParams& param_)
  {
    ...
    // Check we are in 4D
    if ( Nd != 4 ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) only works in 4D" << endl;
      QDP_abort(1);
    }
    
    // Check Aniso Direction -- has to be 3.
    if ( param.anisoParam.anisoP== true && param.anisoParam.t_dir != 3 ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) is hardwired for t_dir=3"<< endl;
      QDP_abort(1);
    }
    ...
    if( t_size[3] != s_size[3] ) { 
      QDPIO::cout << "This class (ILUPrecSCprecTCloverLinOp) needs time to be local" << endl;
      QDP_abort(1);
    }
  ...
  //! Apply (C_L)^{-1}
  void ILUPrecSCprecTCloverLinOp::TPlusOp(T& chi, 
					  const T& psi, 
  ...
  //! Apply (C_R)^{-1}
  void ILUPrecSCprecTCloverLinOp::TMinusOp(T& chi, 
					   const T& psi, 
					   enum PlusMinus isign,
					   int cb3d) const 
  {
  ...
  //! Apply C_L
  void ILUPrecSCprecTCloverLinOp::invTPlusOp(T& chi, const T& psi, enum PlusMinus isign,
					     int cb3d) const
  {
    LatticeHalfFermion tmp_minus;
    LatticeHalfFermion tmp_plus;
  ...
  //! Apply C_R
  void ILUPrecSCprecTCloverLinOp::invTMinusOp(T& chi, const T& psi, enum PlusMinus isign,
					      int cb3d) const
  {
    enum PlusMinus other_sign = isign == PLUS ? MINUS : PLUS ;
    LatticeHalfFermion tmp_minus;
  ...
  void  ILUPrecSCprecTCloverLinOp::derivCRight(P& ds_u, const T& X, const T& Y, enum PlusMinus isign) const 
  {
    T T_1, T_2, T_3, T_4;
    switch(isign) {
    case PLUS: 
  ...
  void  ILUPrecSCprecTCloverLinOp::derivCLeft(P& ds_u, const T& X, const T& Y, enum PlusMinus isign) const
  {
    T T_1, T_2, T_3, T_4;
  ...
  void ILUPrecSCprecTCloverLinOp::derivInvTPlusOp(P& ds_u, const T& X, const T& Y, enum PlusMinus isign) const 
  ...
  void ILUPrecSCprecTCloverLinOp::derivInvTMinusOp(P& ds_u, const T& X, const T& Y, enum PlusMinus isign) const 
  {
