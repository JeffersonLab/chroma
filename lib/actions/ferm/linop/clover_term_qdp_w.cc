// $Id: clover_term_qdp_w.cc,v 3.21 2009-02-04 21:16:03 bjoo Exp $
/*! \file
...
{ 
  ...
  namespace QDPCloverEnv { 
    ...
    struct QDPCloverMakeClovArg {
      ...
      const LatticeColorMatrix& f0;
      const LatticeColorMatrix& f1;
      const LatticeColorMatrix& f2;
      const LatticeColorMatrix& f3;
      const LatticeColorMatrix& f4;
      const LatticeColorMatrix& f5;
    ...
    inline 
    void makeClovSiteLoop(int lo, int hi, int myId, QDPCloverMakeClovArg *a)
    {
      const Real& diag_mass = a->diag_mass;
      const LatticeColorMatrix& f0=a->f0;
      const LatticeColorMatrix& f1=a->f1;
      const LatticeColorMatrix& f2=a->f2;
      const LatticeColorMatrix& f3=a->f3;
      const LatticeColorMatrix& f4=a->f4;
      const LatticeColorMatrix& f5=a->f5;
      multi1d<PrimitiveClovTriang>& tri=a->tri;
  ...
  void QDPCloverTerm::makeClov(const multi1d<LatticeColorMatrix>& f, const Real& diag_mass)
  {
    ...
    QDPCloverEnv::QDPCloverMakeClovArg arg = {diag_mass, f0,f1,f2,f3,f4,f5,tri };
    dispatch_to_threads(nodeSites, arg, QDPCloverEnv::makeClovSiteLoop);
  ...
  void QDPCloverTerm::ldagdlinv(LatticeReal& tr_log_diag, int cb)
  {
    ...
    QDPCloverEnv::LDagDLInvArgs a = { tr_log_diag, tri, cb };
    dispatch_to_threads(rb[cb].numSiteTable(), a, QDPCloverEnv::LDagDLInvSiteLoop);
  ...
  void QDPCloverTerm::chlclovms(LatticeReal& tr_log_diag, int cb)
  {
    ...
    tr_log_diag = zero;
    QDPCloverEnv::CholesArgs a = { tr_log_diag, tri, cb};
    dispatch_to_threads(rb[cb].numSiteTable(), a, QDPCloverEnv::cholesSiteLoop);
  ...
  namespace QDPCloverEnv { 
    ...
    void triaCntrSiteLoop(int lo, int hi, int myId, TriaCntrArgs* a)
    {
      ...
      const multi1d<PrimitiveClovTriang>& tri = a->tri;
      int mat = a->mat;
      int cb  = a->cb;
  ...
  void QDPCloverTerm::triacntr(LatticeColorMatrix& B, int mat, int cb) const
  {
    ...
    QDPCloverEnv::TriaCntrArgs a = { B, tri, mat, cb };
    dispatch_to_threads(rb[cb].numSiteTable(), a, 
			QDPCloverEnv::triaCntrSiteLoop);
...
#warning "Using unrolled clover term - version 2"
			    ...
			    enum PlusMinus isign, int cb) const
  {
    ...
    QDPCloverEnv::ApplyArgs arg = { chi,psi,tri,cb };
    ...
    dispatch_to_threads(num_sites, arg, QDPCloverEnv::applySiteLoop);
