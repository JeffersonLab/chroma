...
namespace Chroma
{
	...
	template<typename T, typename P, typename Q>
		class TwistedShiftedLinOp<T, P, Q, EvenOddPrecLogDetLinearOperator> :
			public DiffLinearOperator<T,P,Q>  {
				public:
				TwistedShiftedLinOp(const EvenOddPrecLogDetLinearOperator<T,P,Q>& base_op_, const Real& mu_):
					base_op(base_op_), mu(mu_){}
				...
				void deriv(P& ds_u, const T& Y, const T& X, enum PlusMinus isign)const override
				{
					...
					const EvenOddPrecConstDetLinearOperator<T,P,Q>& constdet_op =
						static_cast<const EvenOddPrecConstDetLinearOperator<T,P,Q>&>(base_op);
					constdet_op.deriv(ds_u, Y, X, isign);
				}

			private:
				const Real mu;
				const EvenOddPrecLogDetLinearOperator<T,P,Q>& base_op;
		};
}
#endif
