// $Id: overlap_fermact_base_w.cc,v 1.24 2004-12-12 21:22:14 edwards Exp $
/*! \file
...
{ 
			    ...
			    int& n_count) const
  {
	...
	// Source is chiral. In this case we should use InvCG1
	// with the special MdagM
	Handle< const LinearOperator<LatticeFermion> > MM( dynamic_cast<const LinearOperator<LatticeFermion>* >( lMdagM(state, ichiral) ) );
    ...
    case REL_SUMR_INVERTER:
    {
      ...
      Complex zeta = cmplx(( Real(1) + mu ) / (Real(1) - mu),0);
      {
	Handle<const LinearOperator<LatticeFermion> > U( dynamic_cast<const LinearOperator<LatticeFermion>* >(lgamma5epsH(state)) );
    ...
    case REL_GMRESR_SUMR_INVERTER:
    {
      ...
      Complex zeta = cmplx(( Real(1) + mu ) / (Real(1) - mu),0);
      {
	Handle<const LinearOperator<LatticeFermion> > UnprecU( dynamic_cast<const LinearOperator<LatticeFermion>* >(lgamma5epsH(state)) );

	Handle<const LinearOperator<LatticeFermion> > PrecU( dynamic_cast<const LinearOperator<LatticeFermion>* >(lgamma5epsHPrecondition(state)) );
	...
	MM_ptr =  dynamic_cast<const LinearOperator<LatticeFermion>* >( lMdagM(state));
	MM_prec_ptr =  dynamic_cast<const LinearOperator<LatticeFermion>* >( lMdagMPrecondition(state));
	...
	// Source is chiral. In this case we should use InvCG1
	// with the special MdagM
	MM_ptr = dynamic_cast<const LinearOperator<LatticeFermion>* >( lMdagM(state, ichiral) );
	MM_prec_ptr = dynamic_cast<const LinearOperator<LatticeFermion>* >( lMdagMPrecondition(state, ichiral) );
