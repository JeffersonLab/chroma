...
namespace Chroma
{
  ...
  namespace EvenOddPrecCloverOrbifoldFermActEnv
  {
    ...
    {
      return new EvenOddPrecCloverOrbifoldFermAct(CreateFermStateEnv::reader(xml_in, path), 
  ...
  EvenOddPrecLogDetLinearOperator<LatticeFermion,
				  multi1d<LatticeColorMatrix>,
				  multi1d<LatticeColorMatrix> >* 
  EvenOddPrecCloverOrbifoldFermAct::linOp(Handle< FermState<T,P,Q> > state) const
  {
    return new EvenOddPrecCloverOrbifoldLinOp(state,param);
  }
