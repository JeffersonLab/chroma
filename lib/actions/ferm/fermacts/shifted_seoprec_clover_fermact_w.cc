...
namespace Chroma
{
	...
	namespace ShiftSymEvenOddPrecCloverFermActEnv
	{
		...
		WilsonTypeFermAct<LatticeFermion,
			...
			{
				return new ShiftSymEvenOddPrecCloverFermAct(CreateFermStateEnv::reader(xml_in, path), 
		...
		//! Name to be used
		const std::string name = "SHIFTED_SEOPREC_CLOVER";
	...
	SymEvenOddPrecLogDetLinearOperator<LatticeFermion,
		multi1d<LatticeColorMatrix>,
		multi1d<LatticeColorMatrix> >* 
			ShiftSymEvenOddPrecCloverFermAct::linOp(Handle< FermState<T,P,Q> > state) const
			{
				return new ShiftSymEvenOddPrecCloverLinOp(state,param,mu);
			}
