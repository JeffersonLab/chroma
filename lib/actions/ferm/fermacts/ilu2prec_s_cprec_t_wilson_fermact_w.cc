...
namespace Chroma
{
  ...
  namespace ILU2PrecSpaceCentralPrecTimeWilsonFermActEnv
  {
    ...
    {
      return new ILU2PrecSpaceCentralPrecTimeWilsonFermAct(CreateFermStateEnv::reader(xml_in, path), 
    ...
    //! Name to be used
    const std::string name = "ILU2PREC_SPACE_CPREC_TIME_WILSON";
  ...
  ILU2PrecSpaceCentralPrecTimeLinearOperator<LatticeFermion,
		       multi1d<LatticeColorMatrix>,
		       multi1d<LatticeColorMatrix> >*
  ILU2PrecSpaceCentralPrecTimeWilsonFermAct::linOp(Handle< FermState<T,P,Q> > state) const
  {
    return new ILU2PrecSCprecTWilsonLinOp(state,param.Mass,param.anisoParam); 
  }
