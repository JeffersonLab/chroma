...
namespace Chroma
{
  ...
  namespace MdagMSysSolverMRHSProxyEnv
  {
    ...
    MdagMMRHSSystemSolver<LatticeFermion>* createFerm(XMLReader& xml_in,
						  ...
						  Handle< FermState<
						                     ...
						                     multi1d<LatticeColorMatrix> 
					 	  	  	  	  	   > > state)
    {
      return new MdagMMRHSSysSolverProxy<LatticeFermion,multi1d<LatticeColorMatrix>,
    ...
    bool registerAll() 
    {
      ...
      if (! registered)
      {
	success &= Chroma::TheMdagMFermMRHSSystemSolverFactory::Instance().registerObject(name, createFerm);
#if 0
