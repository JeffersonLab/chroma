...
namespace Chroma {
  ...
  SysSolverQUDAMULTIGRIDWilsonParams::SysSolverQUDAMULTIGRIDWilsonParams(XMLReader& xml, 
						       const std::string& path)
  {
    ...
    if ( paramtop.count("AsymmetricLinop") > 0 ) { 
      ...
      asymmetricP = true; // Asymmetric is default although it doesn't matter here
    }
  ...
  {
    SysSolverQUDAMULTIGRIDWilsonParams tmp(xml, path);
    p = tmp;
  }
