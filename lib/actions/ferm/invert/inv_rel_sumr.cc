...
//
// This is now with Relaxation in the solver...
//
//
		  ...
		  int& n_count)
{
  ...
  for(int iter = 1; iter <= MaxSUMR && !convP; iter++) {
    ...
    // taumod is a bound on || r || 
    // inner solver criterion is epsilon/|| r ||

    Real inner_tol = epsilon/taumod;
    ...
    if( toBool( sqrt(real(conj(sigma)*sigma)) > epsilon) ) {
      ...
      //
      taumod = sqrt(real(conj(tauhat)*tauhat));
...
void InvRelSUMR(const LinearOperator<LatticeFermion>& U,
		...
		int& n_count)
{
  InvRelSUMR_a(U, b, x, zeta, rho, epsilon, MaxSUMR, n_count);
}
