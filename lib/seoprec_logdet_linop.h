...
namespace Chroma 
{
   ...
   *
   * where A^{-1}_{ee} and A^{-1}_(o,o) are dependent of the gauge fields. 
   * We can exactly simulate the determinant without pseudofermion fields
   ...
   * 
   * hence we have functions  logDetEvenEvenLinOp() and logDetOddOddLinOp()
   *  and                     derivEvenEvenLinOp()
   *  and                     derivLogDetEvenEvenLinOp()
   *  and                     derivLogDetOddOddLinOp()
   */
  ...
  public:
		       ...
		       enum PlusMinus isign) const
    {
      ...
      //
      T   tmp1, Dpsi, ADpsi, DADpsi, Achi, DAchi, ADAchi;
      ...
      // DADpsi = D_oe Ainv_ee D_eo psi_o
      this->oddEvenLinOp(DADpsi, ADpsi, isign);
      ...
      //  ds_u  +=  chi^dag * A_oo * A'_oo * A_oo * D_oe * Ainv_ee * D_eo * psi_o
      this->oddOddInvLinOp(tmp1, DADpsi, isign);
      this->derivOddOddLinOp(ds_u, Achi, tmp1, isign);

      //  ds_u  -=  chi^dag * Ainv_oo * D'_oe * Ainv_ee * D_eo * psi_o
      this->derivOddEvenLinOp(ds_1, Achi, ADpsi, isign);
      ds_u -= ds_1;

      //  ds_u  +=  chi^dag * Ainv_oo * D_oe * Ainv_ee * A'_ee * Ainv_ee * D_eo * psi_o
      this->derivEvenEvenLinOp(ds_1, ADAchi, ADpsi, isign);
      ds_u += ds_1;
	...
	//  ds_u  +=  chi^dag * A_oo * A'_oo * A_oo * D_oe * Ainv_ee * D_eo * psi_o
	this->oddOddInvLinOp(tmp1, DADpsi, isign);
	this->derivOddOddLinOp(ds_1, Achi, tmp1, isign);
	ds_u += ds_1;
    ...
    virtual void derivOddOddLinOp(P& ds_u, const T& chi, const T& psi, 
				  enum PlusMinus isign) const
    {
      QDPIO::cerr << "OddOdd: not implemented" << std::endl;
      QDP_abort(1);
    }
    ...
    virtual void derivLogDetOddOddLinOp(P& ds_u, enum PlusMinus isign) const
    {
      QDPIO::cerr << "derivEvenEven: not implemented" << std::endl;
      QDP_abort(1);
    }
  ...
  template<typename T, typename P, typename Q>
  class SymEvenOddPrecLogDetLinearOperatorArray : public SymEvenOddPrecLinearOperatorArray<T,P,Q>
  {
  public:
    //! Virtual destructor to help with cleanup;
    virtual ~SymEvenOddPrecLogDetLinearOperatorArray() {}
